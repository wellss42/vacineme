...
class PostosDetalhesViewController: UIViewController, MKMapViewDelegate {
    
    var coordinator: Coordinator?
    let detalhesPosto: Estabelecimento
    ...
    private lazy var adicionarButton: UIButton = {
        ...
        button.addTarget(self, action: #selector(adicionar), for: .touchUpInside)
        return button
    }()
    
    init(detalhesPosto: Estabelecimento) {
    ...
    func setupMap() {
        ...
        if (locationManager?.location?.coordinate) != nil {
            ...
            let coordenadaLocal2 = CLLocationCoordinate2D(latitude: detalhesPosto.latitudeEstabelecimentoDecimoGrau ?? 0.0, longitude: detalhesPosto.longitudeEstabelecimentoDecimoGrau ?? 0.0)
    ...
    @objc func adicionar() {
        ...
        let data: [String : Any] = [
            "nome_fantasia": detalhesPosto.nomeFantasia,
            "descricao_turno_atendimento": detalhesPosto.descricaoTurnoAtendimento,
            "numero_telefone_estabelecimento": detalhesPosto.numeroTelefoneEstabelecimento ?? "",
            "endereco_estabelecimento": detalhesPosto.enderecoEstabelecimento,
            "numero_estabelecimento": detalhesPosto.numeroEstabelecimento,
            "bairro_estabelecimento": detalhesPosto.bairroEstabelecimento,
            "codigo_cep_estabelecimento": detalhesPosto.codigoCepEstabelecimento,
            "latitude_estabelecimento_decimo_grau": detalhesPosto.latitudeEstabelecimentoDecimoGrau ?? 0.0,
            "longitude_estabelecimento_decimo_grau": detalhesPosto.longitudeEstabelecimentoDecimoGrau ?? 0.0,
            "id_mobile": deviceId ?? ""
        ]
        
        Firestore.firestore().collection(cidade ?? "Outros").document(detalhesPosto.nomeFantasia).setData(data) { error in
            if let error = error {
        ...
        coordinator?.navigate(to: .homeKill, data: nil)
    }
    
    func alert() {
        let alert = UIAlertController(title: "Atenção", message: "Dados Adicionados com sucesso!", preferredStyle: .alert)
    ...
    func updateScreen() {
        DispatchQueue.main.async{[ weak self] in
            self?.titleLabel.text = self?.detalhesPosto.nomeFantasia
            self?.enderecoLabel.text = "Endereço: \(self?.detalhesPosto.enderecoEstabelecimento ?? "")"
            self?.numeroLabel.text = "Número: \(self?.detalhesPosto.numeroEstabelecimento ?? "")"
            self?.bairroLabel.text = "Bairro: \(self?.detalhesPosto.bairroEstabelecimento ?? "")"
            self?.cepLabel.text = "Cep: \(self?.detalhesPosto.codigoCepEstabelecimento ?? "")"
            self?.telefoneLabel.text = "Telefone: \(self?.detalhesPosto.numeroTelefoneEstabelecimento ?? "")"
        }
        titleLabel.text = detalhesPosto.nomeFantasia
    }
    
}

extension PostosDetalhesViewController: CLLocationManagerDelegate {
...
extension PostosDetalhesViewController: ViewCoding {
    ...
    func buildViewHerarchy() {
        ...
        bodyView.addSubview(adicionarButton)
    }
    ...
    func setupContraints() {
        ...
        NSLayoutConstraint.activate([
            ...
            scrollView.bottomAnchor.constraint(equalTo: adicionarButton.topAnchor),
            ...
            adicionarButton.heightAnchor.constraint(equalToConstant: 50),
            adicionarButton.bottomAnchor.constraint(equalTo: bodyView.bottomAnchor, constant: -20),
            adicionarButton.leadingAnchor.constraint(equalTo: bodyView.leadingAnchor, constant: 16),
            adicionarButton.trailingAnchor.constraint(equalTo: bodyView.trailingAnchor, constant: -16),
